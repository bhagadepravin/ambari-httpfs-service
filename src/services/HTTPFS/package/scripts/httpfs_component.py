...
class Httpfs_component(Script):
    ...
    def stop(self, env):
        ...
        Execute("/usr/odp/current/hadoop-httpfs/etc/rc.d/init.d/hadoop-httpfs stop")
    def start(self, env):
        import params
        self.configure(env)
        Logger.info("Starting HttpFS service")
        Execute("/usr/odp/current/hadoop-httpfs/etc/rc.d/init.d/hadoop-httpfs start")
    def status(self, env):
        Logger.info("Getting status of HttpFS service")
        try:
            Execute("/usr/odp/current/hadoop-httpfs/etc/rc.d/init.d/hadoop-httpfs status")
        except Fail:
